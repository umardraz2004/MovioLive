...
} from "../utils/sendSubscriptionEmail.js";
    ...
    for (const user of expiredSubUsers) {
      try {
        ...
        if (user.subscriptionId) {
          ...
          console.log(
            `Canceled expired subscription in Stripe: ${user.subscriptionId} for user: ${user.email}`
          );
        }
        ...
        expiredCount++;
        console.log(
          `Completely cleaned up expired subscription data for user: ${user.email}`
        );
      } catch (stripeError) {
        console.error(
          `Error canceling subscription ${user.subscriptionId}:`,
          stripeError
        );
        ...
        expiredCount++;
        console.log(
          `Cleaned up database for user: ${user.email} (Stripe cancellation failed)`
        );
      }
    }
...
const handleCheckoutCompleted = async (session) => {
  // Check if this is a ticket purchase or subscription
  if (session.metadata.paymentType === "ticket") {
    ...
    try {
      ...
      console.log(
        `Added Organizer role to user: ${user.email} (${planType} - ${billingPeriod})`
      );
    } catch (error) {
...
// Handle ticket purchase from webhook
const handleTicketPurchase = async (session) => {
  const { userId, eventId } = session.metadata;

  try {
    console.log(`Processing ticket purchase for event ${eventId}`);

    // Get event details to get the price
    const event = await Event.findById(eventId);
    if (!event) {
      console.error(`Event not found for ticket purchase: ${eventId}`);
      return;
    }
    ...
    if (user) {
      ...
      if (user.subscriptionId && user.subscriptionId !== subscription.id) {
        try {
          await stripe.subscriptions.cancel(user.subscriptionId);
          console.log(
            `Canceled previous subscription: ${user.subscriptionId} for user: ${user.email}`
          );
        } catch (cancelError) {
          console.error(
            `Error canceling previous subscription ${user.subscriptionId}:`,
            cancelError
          );
        }
      }
      ...
      if (
        ...
        planName =
          firstItem.price?.nickname || firstItem.plan?.nickname || null;
      }
      ...
      if (isDailyPlan) {
        try {
          ...
          console.log(
            `Set daily subscription to cancel at period end: ${subscription.id}`
          );
        } catch (stripeError) {
          console.error(
            `Error setting cancel_at_period_end for ${subscription.id}:`,
            stripeError
          );
        }
      }
...
const handleSubscriptionDeleted = async (subscription) => {
  try {
    ...
    if (user) {
      ...
      console.log(
        "Subscription deleted and all fields reset for user:",
        user._id
      );
    } else {
      console.warn("User not found for subscriptionId:", subscription.id);
...
export const getCheckoutSession = async (req, res) => {
  try {
    ...
    const enrichedSession = {
      ...session,
      payment_name:
        userPlanName || session.metadata?.planType || "Premium Plan",
    };
  ...
  try {
    ...
    if (user.subscriptionId) {
      try {
        await stripe.subscriptions.cancel(user.subscriptionId);
        console.log(
          "Successfully canceled subscription in Stripe:",
          user.subscriptionId
        );
      } catch (stripeError) {
        console.error(
          "Stripe cancellation failed (but continuing with DB cleanup):",
          stripeError.message
        );
...
export const createTicketCheckoutSession = async (req, res) => {
  try {
    ...
    console.log("Received eventId for ticket session:", eventId);
    const userId = req.user.id;
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              ...
              description: `Movie event on ${event.date} at ${event.time}`,
            },
      ...
      success_url: successUrl,
      cancel_url: `${process.env.BASE_URL}/events`,
      metadata: {
        paymentType: "ticket",
        userId: userId,
        eventId: eventId,
